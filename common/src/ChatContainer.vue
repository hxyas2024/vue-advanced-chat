    ...
    async fetchMoreRooms() {
      ...
      roomsData.forEach(room => {
        room.users.forEach(userId => {
          // ?.表示不为null或undefined，则进一步访问_id，否则返回undefined
          // 在用户列表中找到该用户，如果没找到，就放进roomUserIds
      ...
      // 获取房间名称
      await request({
        url: 'http://localhost:8082/im/user/test/user_details',
        method: 'GET',
        params: {
          userIds: roomUserIds
        }
          ...
          if (response.code === 200) {
            // 查找用户信息，使用username作为群名，且会有_id,如果为空，就使用Myself
      ...
      const roomList = {}
      roomsData.forEach(room => {
        // key为room.id，并置空users,其目的是为了过滤掉被删除或异常的用户
      ...
      const listener = firestoreService.listenRooms(query, rooms => {
        ...
        rooms.forEach(room => {
          ...
          if (foundRoom) {
            ...
            foundRoom.index = room.lastUpdated
          }
        })
      })
      this.roomsListeners.push(listener)
